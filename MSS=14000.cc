...
{
  *stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << (oldCwnd/14000) << "\t" << (newCwnd/14000) << std::endl;
...
//Slow Start Threshold
  ...
  Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (14000));
...
sink = StaticCast<PacketSink> (sinkApps.Get (0));
     ...
     //Simulator::Schedule (Seconds (0), &errorrate);
      ...
      app->Setup (ns3TcpSocket, sinkAddress, 14000, 5000000, DataRate ("1000Mb/s"));
...
//
     Simulator::Schedule (Seconds (0), &CalculateThroughput);
      ...
      Ptr<MyApp> app = CreateObject<MyApp> ();
      app->Setup (ns3UdpSocket, sinkAddress, 14000, 5000000, DataRate ("1000Mb/s"));
